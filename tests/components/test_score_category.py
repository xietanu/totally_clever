...
def test_center_x():
    ...
    score_cat = ScoreCategory("images/GreenCategoryBox.png", 0, 0)
    assert score_cat.center_x == 250
    score_cat = ScoreCategory("images/green_icon.png", 100, 100)
...
def test_score_trackers_created():
    ...
    score_cat = ScoreCategory(
        "images/GreenCategoryBox.png", 0, 0, score_mode=ScoreModes.SUM
    )
    assert score_cat.score_trackers is None
    score_cat = ScoreCategory(
        "images/GreenCategoryBox.png", 0, 0, score_mode=ScoreModes.MATCHED
    )
    assert isinstance(score_cat.score_trackers, SpriteList)
    score_cat = ScoreCategory(
        "images/GreenCategoryBox.png", 0, 0, score_mode=ScoreModes.COUNT
    )
    assert isinstance(score_cat.score_trackers, SpriteList)
...
def test_add_box_added_to_boxes():
    ...
    score_cat = ScoreCategory("images/GreenCategoryBox.png", 0, 0)
    score_cat.add_box(0, 0)
    assert len(score_cat.boxes) == 1
    score_cat = ScoreCategory("images/GreenCategoryBox.png", 0, 0)
    score_cat.add_box(0, 0)
    score_cat.add_box(10, 0)
...
def test_add_box_position_x():
    ...
    score_cat = ScoreCategory("images/GreenCategoryBox.png", 100, 50)
    score_cat.add_box(50, 10)
